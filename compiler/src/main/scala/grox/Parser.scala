...
object Parser:
  ...
  // and its OPERATOR is ("==" | "!=").
  def binary(operators: List[Operator], descendant: List[Token] => ExprTokens)(tokens: List[Token]): ExprTokens = 
  ...
  def unary(tokens: List[Token]): ExprTokens = tokens match
  ...
  def parenBody(tokens: List[Token]): ExprTokens = expression(tokens) match
      case (Left(e), rest) => (Left(e), rest)
      case (Right(expr), rest) => rest match 
