...
class StmtParserTest extends munit.FunSuite {
  ...
  test("Parse full variable declaration") {
    val ts = List(Keyword.Var, Literal.Identifier("x"), Operator.Equal, Literal.Number("1"), Operator.Semicolon)
    val want = Stmt.Var(Literal.Identifier("x"), Some(Expr.Literal(1)))
    assertEquals(parseStmt(Inspector(List.empty, List.empty, ts)), Inspector(List.empty, List(want), List.empty))
  }

  test("Parse empty token list") {
    val ts = List()
    assertEquals(parseStmt(Inspector(List.empty, List.empty, ts)), Inspector(List.empty, List.empty, List.empty))
  }
  ...
  test("Parse full variable declaration with trailing expressions") {
    new TestSets:
      val ts = List(Keyword.Var, Literal.Identifier("x"), Operator.Equal, Literal.Number("1"), Operator.Semicolon, num2, Operator.Star, num3, Operator.Semicolon)
      val want = List(Stmt.Var(Literal.Identifier("x"), Some(Expr.Literal(1.0))), Stmt.Expression(Expr.Multiply(expr2, expr3)))
      assertEquals(
  ...
  test("Parse single print Statement") {
    new TestSets:
      ...
      assertEquals(parseStmt(Inspector(List.empty, List.empty, ts)), Inspector(List.empty, want, List.empty))
  }
