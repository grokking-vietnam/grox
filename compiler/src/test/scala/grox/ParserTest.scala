...
class ParserTest extends munit.FunSuite:
  ...
  test("primary bool") {
    val ts = List(Keyword.True, Keyword.False)
    val (first, rest1) = parse(ts)
    assertEquals(first, Right(Expr.Literal(true)))

    val (sec, rest2) = parse(rest1)
    assertEquals(sec, Right(Expr.Literal(false)))
  }
