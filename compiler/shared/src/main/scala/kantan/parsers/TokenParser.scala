...
/** Optimised version of [[Parser]] when working at the token level.
  *
  * Knowing that we're working directly with the input allows us, for example, to treat repetition
  * operators as ranging over an array rather than the default step-by-step approach.
  *
  * For example:
  * {{{
  ...
  * }}}
  *
  * [[TokenParser]] will treat this as extracting the sub-sequence from index 0 to 3 in `567`,
  * rather than parsing `5`, `6` and `7` individually, and accumulate them in a list.
  */
private class TokenParser[Token: SourceMap](
  parse: State[Token] => Result[Token, Token],
  pred: Token => Boolean,
  ...
  override def label(label: String): TokenParser[Token] =
    new TokenParser(state => run(state).label(label), pred)
  override def backtrack = new TokenParser(state => run(state).empty, pred)

  // - Repetition ------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------
  def rep(allowEmpty: Boolean): Parser[Token, List[Token]] =
    state => {
      ...
      if (start < stop)
        ...
        Result.Ok(false, Parsed(List.empty, state.pos, state.pos), state, Message.empty)
      else
        Result.Error(false, Message(state, List.empty))
    }
...
private[parsers] object TokenParser:
  ...
  def apply[Token: SourceMap](pred: Token => Boolean): TokenParser[Token] =
    new TokenParser(
      state =>
        if (state.isEOF)
          Result.Error(false, Message(state, List.empty))
