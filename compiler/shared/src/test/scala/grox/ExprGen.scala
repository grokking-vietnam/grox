...
object ExprGen:
  type BinOperator = (Expr[Unit], Expr[Unit]) => Expr[Unit]

  type Term = Expr.Add[Unit] | Expr.Subtract[Unit]
  type Factor = Expr.Multiply[Unit] | Expr.Divide[Unit]
  type Equality = Expr.Equal[Unit] | Expr.NotEqual[Unit]

  def group[T: Typeable](expr: Expr[Unit]): Expr[Unit] =
    expr match
      case _: T => Expr.Grouping(expr)
      case _    => expr

  def binaryGen(operator: BinOperator)(operand: => Gen[Expr[Unit]]): Gen[Expr[Unit]] =
    for
      left <- operand
      right <- operand
    yield operator(left, right)

  def treeGen(operator: BinOperator, operand: => Gen[Expr[Unit]]): Gen[Expr[Unit]] = Gen.sized(size =>
    for
  ...
  def addOperator(left: Expr[Unit], right: Expr[Unit]): Expr[Unit] = Expr.Add(left, right)

  def subtractOperator(left: Expr[Unit], right: Expr[Unit]): Expr =
    val gRight = group[Term](right)
    Expr.Subtract(left, gRight)

  def multiplyOperator(left: Expr[Unit], right: Expr[Unit]): Expr =
    val gLeft = group[Term](left)
    val gRight = group[Term](right)
    Expr.Multiply(gLeft, gRight)

  def divideOperator(left: Expr[Unit], right: Expr[Unit]): Expr =
    val gLeft = group[Term](left)
    val gRight = group[Term | Factor](right)
    Expr.Divide(gLeft, gRight)

  def negateOperator(left: Expr[Unit]): Expr =
    left match
  ...
  val negateGen: Gen[Expr[Unit]] = Gen.sized(size =>
    for expr <- Gen.resize(size - 1, numericGen)
    yield negateOperator(expr)
  )

  val numericGen: Gen[Expr[Unit]] = Gen.sized(size =>
    if (size == 0)
  ...
  def equalOperator(left: Expr[Unit], right: Expr[Unit]): Expr[Unit] =
    val gLeft = group[Equality](left)
    val gRight = group[Equality](right)
    Expr.Equal(gLeft, gRight)

  def notEqualOperator(left: Expr[Unit], right: Expr[Unit]): Expr[Unit] =
    val gLeft = group[Equality](left)
    val gRight = group[Equality](right)
    Expr.NotEqual(gLeft, gRight)

  def notOperator(left: Expr[Unit]): Expr[Unit] =
    left match
      case _: Expr.Literal => Expr.Not(left)
      case _               => Expr.Not(Expr.Grouping(left))

  val equalityGen: Gen[Expr[Unit]] = Gen.oneOf(
  ...
  val comparisonGen: Gen[Expr[Unit]] = Gen.oneOf(
    treeGen(((x, y) => Expr.Greater((), x y)), numericGen),
  ...
  val logicalGen: Gen[Expr[Unit]] = Gen.sized(size =>
    if (size == 0)
