...
/** State of a parser.
  ...
  *
  * TODO: before writing documentation, we need to keep track of a token's START and END position.
  * It makes things far easier to explain. With chars, a token's start position is always the
  * previous token's end position. With more complex tokens, this might not hold - think of
  * space-separated ints: "1 2". You cannot guess the start position of '2' just from '1': this
  * doesn't tell you how many spaces there are before the next token starts.
  */
case class State[Token: SourceMap](input: IndexedSeq[Token], offset: Int, pos: Position):
  ...
  def consume(parsed: Token): State[Token] = copy(
    offset = offset + 1,
    pos = SourceMap[Token].endsAt(parsed, pos),
  )
...
object State:
  ...
  def init[Token: SourceMap](tokens: IndexedSeq[Token]): State[Token] = State(
    tokens,
    0,
    Position.zero,
  )
